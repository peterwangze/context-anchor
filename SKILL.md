...
- 后台巡检可通过用户选择的方式配置为 Windows Task Scheduler、macOS launchd 或 Linux systemd user timer
...
- `heartbeat`：不是 OpenClaw managed hook 事件；通过 `scripts/heartbeat.js` 或后台巡检任务触发

安装脚本：

```bash
node scripts/install-one-click.js [--openclaw-home <openclaw-home>] [--skills-root <skills-root>] [--yes] [--keep-memory|--drop-memory] [--apply-config|--skip-config] [--enable-scheduler|--skip-scheduler] [--target-platform <windows|macos|linux>] [--workspace <workspace>] [--interval-minutes <minutes>]
```

单独重跑配置：

```bash
node scripts/configure-host.js [--openclaw-home <openclaw-home>] [--skills-root <skills-root>] [--yes] [--apply-config|--skip-config] [--enable-scheduler|--skip-scheduler] [--target-platform <windows|macos|linux>] [--workspace <workspace>] [--interval-minutes <minutes>]
```
...
- 写入 workspace 巡检 wrapper 到 `<openclaw-home>/automation/context-anchor/`
- 如果检测到旧安装，会先清理旧安装文件再重装
- 如果检测到旧记忆，会询问用户是否保留记忆数据
- 可选写入推荐 `openclaw.json` 配置
- 安装到默认托管目录时不需要额外写 `skills.load.extraDirs`
- 只有自定义 `skills-root` 时才会补写 `skills.load.extraDirs`
- 可选为某个 workspace 启用后台巡检任务，并选择 Windows / macOS / Linux 的调度方式
...
- 未配置时默认 `user_id = default-user`
- 用户级数据写入 `<openclaw-home>/context-anchor/users/<user-id>/`
- 一键配置会在 `<openclaw-home>/context-anchor-host-config.json` 登记默认用户、workspace 和 session 归属
...
- `command:new`、`command:reset`、`command:stop` 都会走统一的 `session-close` 闭环
- `heartbeat` 和 `session-close` 都会触发 scoped skill promotion
- 同名技能按 `session > project > user` 做冲突裁决
- `inactive/archived` skill 不进入自动激活集合
- 同名 active skill 晋升时优先复用已有 scoped skill，而不是重复创建
- `skill-reconcile` 会根据 source experience 状态自动降级失效 skill
- scoped skill 保留 `promotion_history` 和 `status_history`
- `skill-supersede` 可显式声明 supersede 关系
- skill 激活集合受预算治理约束，超出预算的 skill 不会进入 `effective_skills`
- 长期低价值且 inactive 的 skill 会在 reconcile 中自动 archive
...
- same-name scoped skill governance
- 经验会先校验再技能化
- skill 创建落到目标技能目录
- `agent:bootstrap` 会把持久化上下文注入真实 OpenClaw session
- 未登记 workspace 不会自动归属，而是注入明确的配置引导
- host 安装脚本会写 wrapper，并在需要时写入 `openclaw.json`
