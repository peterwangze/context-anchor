...
**触发条件：**
- 上下文压力 > 75%（自动触发）
- 上下文压力 > 85%（自动触发 + 建议 compact）
- 用户请求保存（"保存记忆"、"记住这个"、"创建检查点"）
- 重要决策/发现时
- Session 结束前
...
| 使用率 | 自动操作 | 用户提示 |
|--------|----------|----------|
| **> 75%** | 保存高热度记忆到 `memory-hot.json`，创建 `checkpoint.md` | 无 |
| **> 85%** | 执行上述 + 自动执行 `/compact` | "上下文压力较高，已自动压缩并保存记忆" |
| **> 90%** | 执行上述 + 强制保存关键信息 | "⚠️ 上下文压力过高，已保存关键记忆，建议精简对话" |
...
```
1. 使用 session_status 检查上下文使用率
2. 如果 > 75%：
   - 提醒用户："⚠️ 上下文压力较高（XX%），建议保存记忆或执行 /compact"
   - 询问是否需要创建检查点
3. 如果 > 85%：
   - 强制提醒："🚨 上下文压力过高（XX%），已接近上限"
   - 建议用户执行 /compact
4. 如果 > 90%：
   - 警告："🔴 上下文即将溢出，请立即执行 /compact 或开始新会话"
```
...
1. **每次回复前** → 检查上下文压力，必要时提醒用户
2. **Session Start** → 加载项目和全局记忆，检查旧格式迁移
3. **Heartbeat** → 评估热度、检查承诺、触发流动、技能化建议
4. **任务前** → 扫描可用工具，**安全检查**，优先使用现成技能
5. **危险操作** → **必须向用户确认，说明风险**
6. **任务后** → 总结工具使用，记录到 experiences.json
7. **重要决策** → 写入项目级 decisions.json
8. **用户偏好** → 写入项目级 state.json
9. **错误捕获** → 自动记录到 experiences.json (type: lesson)
10. **学习记录** → 发现最佳实践时记录到 experiences.json
11. **功能请求** → 用户请求不存在功能时记录
12. **Session End** → 同步到项目级，更新热度索引
