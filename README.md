...
对于这种 `COMPLETE` 的参考连续性，bootstrap 里的 `Recovered Continuity` 现在也会更明确说明：这是一条“已完成任务的参考连续性”，系统不会再把旧 goal / next step 当成新的活动任务继续恢复。
...
`sessions-diagnose` 现在也会同步显示同一组 hidden filter / inspect / cleanup / next-step 信息；如果当前可见 session 本身没异常、但 profile 里仍有 hidden residue，诊断结果也不会再误报成 “No session anomalies detected.”。
...
对于这种“批量 drift repair”链路，`Auto fix command` 现在也会自动沿用 drift 流程的低感知默认策略：优先执行 repair，默认跳过立即 recheck，避免你在一串批量迁移之后还要立刻承受一次额外回检打断。
...
现在 `configure-host.js`、`configure-sessions.js`、`status-report.js`、`upgrade-sessions.js` 也开始复用同一套 remediation 文本渲染：`Next step`、`Auto fix`、`Auto fix unavailable`、`Resume command`、`Suggested resume`、`Resume checks` 等标签会保持一致，减少不同入口之间的阅读切换成本。
...
从这一轮开始，`Suggested resume` 的多候选排序还会参考你最近已经确认过的 `workspace`、`session-key`、`project-id`、`user-id` 和 profile 选择：如果系统发现某个候选更接近你的确认历史，会优先把它放到前面，并在 `Suggested inputs` / `Suggested checks` 里明确提示这是“基于确认历史的 preferred candidate”。
